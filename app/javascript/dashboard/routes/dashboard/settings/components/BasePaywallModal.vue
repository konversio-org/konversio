...
<template>
  ...
  >
    ...
    <p class="text-sm font-normal text-n-slate-11">
      ...
      <span v-if="isOnKonversioCloud && !isAdmin">
        {{ $t('GENERAL_SETTINGS.LIMIT_MESSAGES.NON_ADMIN') }}
      </span>
      <span v-else-if="!isOnKonversioCloud && !isSuperAdmin">
        {{ $t(`${featurePrefix}.ENTERPRISE_PAYWALL.ASK_ADMIN`) }}
      </span>
    </p>
    <template v-if="isOnKonversioCloud && isAdmin">
    ...
    <template v-else-if="!isOnKonversioCloud && isSuperAdmin">
