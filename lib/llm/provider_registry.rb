...
module Llm::ProviderRegistry
  ...
  REASONING_CAPABILITIES = {
    scaleway: [
      {
        model_pattern: /gemma/i,
        supported: true,
        style: :effort_enum,
        parameter: :reasoning_effort,
        valid_levels: %w[off low medium high],
        intent_map: {
          'off' => 'none',
          'low' => 'low',
          'medium' => 'medium',
    ...
    anthropic: [
      {
        model_pattern: /claude-3-7-sonnet/i,
        supported: true,
        style: :token_budget,
        parameter: :thinking,
        valid_levels: %w[off low medium high],
  ...
  class << self
    def declared_reasoning_supported?(provider, model)
      provider = provider.to_s.downcase.to_sym
      model_name = model.to_s
      if REASONING_CAPABILITIES.key?(provider)
        match = REASONING_CAPABILITIES[provider].find { |cap| model_name =~ cap[:model_pattern] }
        return true if match && match[:supported]
      end
      false
    end

    # rubocop:disable Metrics/MethodLength
    def reasoning_capability(provider, model)
      provider = provider.to_s.downcase.to_sym
      model_name = model.to_s
      ...
      # 2. Check live-probed cache
      if Llm::SanityTester.reasoning_supported_cached?(provider, model_name)
        # Fallback reasoning config for dynamically discovered models (OpenAI-compatible)
        return {
          supported: true,
          style: :effort_enum,
          parameter: :reasoning_effort,
