...
describe Webhooks::Trigger do
  ...
  describe 'request headers' do
    ...
    context 'with delivery_id' do
      it 'adds X-Konversio-Delivery header' do
        expect(RestClient::Request).to receive(:execute) do |args|
          expect(args[:headers]['X-Konversio-Delivery']).to eq('test-uuid')
          expect(args[:headers]).not_to have_key('X-Konversio-Signature')
          expect(args[:headers]).not_to have_key('X-Konversio-Timestamp')
        end
    ...
    context 'with secret' do
      ...
      it 'adds X-Konversio-Timestamp header' do
        expect(RestClient::Request).to receive(:execute) do |args|
          expect(args[:headers]['X-Konversio-Timestamp']).to match(/\A\d+\z/)
        end
        trigger.execute(url, payload, webhook_type, secret: secret)
      end

      it 'adds X-Konversio-Signature header with correct HMAC' do
        expect(RestClient::Request).to receive(:execute) do |args|
          ts = args[:headers]['X-Konversio-Timestamp']
          expected_sig = "sha256=#{OpenSSL::HMAC.hexdigest('SHA256', secret, "#{ts}.#{body}")}"
          expect(args[:headers]['X-Konversio-Signature']).to eq(expected_sig)
        end
        trigger.execute(url, payload, webhook_type, secret: secret)
      end
      ...
      it 'signs timestamp.body not just body' do
        expect(RestClient::Request).to receive(:execute) do |args|
          args[:headers]['X-Konversio-Timestamp']
          wrong_sig = "sha256=#{OpenSSL::HMAC.hexdigest('SHA256', secret, body)}"
          expect(args[:headers]['X-Konversio-Signature']).not_to eq(wrong_sig)
        end
        trigger.execute(url, payload, webhook_type, secret: secret)
      end
    end
    ...
    context 'with both secret and delivery_id' do
      it 'includes all three security headers' do
        expect(RestClient::Request).to receive(:execute) do |args|
          expect(args[:headers]['X-Konversio-Delivery']).to eq('abc-123')
          expect(args[:headers]['X-Konversio-Timestamp']).to be_present
          expect(args[:headers]['X-Konversio-Signature']).to start_with('sha256=')
        end
